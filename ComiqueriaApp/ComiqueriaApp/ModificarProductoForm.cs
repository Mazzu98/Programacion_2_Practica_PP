    ...
    public partial class ModificarProductoForm : Form
    {
        ...
        private void OnModificarClick(object sender, EventArgs e)
        {
            ...
            if (Double.TryParse(this.txtNuevoPrecio.Text, out nuevoPrecio))
            {
                ...
                {
                    AccesoDatos ac = new AccesoDatos(@"Data Source=localhost\SQLEXPRESS;Initial Catalog=Comiqueria;Integrated Security=True");
                    ac.ModificarDato(float.Parse(nuevoPrecio.ToString()), this.productoSeleccionado.Codigo);
